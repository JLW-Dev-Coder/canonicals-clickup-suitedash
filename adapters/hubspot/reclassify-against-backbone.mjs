...
export const reclassify = (form) => {
  ...
  for (const [g, def] of Object.entries(MAP433A.groups || {})) {
    ...
    // The eight 433-A groups with no row_class are exactly the eight classes asset-row-shapes
    // declares `unrouted` on 433-A (see assert-row-shape-spec [A3] and _carried D-05), so the
    // class is read from the spec's own routing declaration rather than guessed from the name.
    const cls = (SHAPES.classes || []).find(c => c.unrouted?.['433a'] && new RegExp(`groups\\.${g}\\b`).test(String(c.unrouted['433a'])));
    if (cls) classOfGroup433a.set(g, [cls.class_id]);
  }
  const asymmetric = [];
  for (const e of CLS.entries) {
  ...
  return { form, back, reuse, candidates, comparedAgainst, asymmetric, granularity, sameConstruct, problems, CLS, coverage };
};
...
export const report = (r, { verbose = false } = {}) => {
  ...
  for (const c of r.candidates.filter(c => c.ruling === 'same-fact')) console.log(`    SAME FACT  ${c.key} = ${c.candidate} [${c.contributors.join('+')}] — carried as ${c.carried}, not rebound`);
