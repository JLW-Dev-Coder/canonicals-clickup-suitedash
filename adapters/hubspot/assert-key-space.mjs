...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const CASES = [
    ...
    { name: 'a `_binds` row-level construct is NOT a member; the group source key carries the fact',
      map: { form: 'zz', groups: { rows: { source: 'rows', slots: [{}] } }, checkboxes: { _binds: { flag: { group: 'rows', column: 'is_x' } }, flag: ['T1', 'T2'] } },
      wanted: ['rows'] },
    ...
    { name: 'a group with NO slot list contributes nothing — it is not a repeatable table the engine feeds',
    map: { form: 'zz', groups: { rows: { source: 'rows' } }, map: { a: 'T' } },
    wanted: ['a'] },
  { name: 'a composite source key is a member — the block validate-crosswalk.mjs read and this selector did not',
    map: { form: 'zz', special: { composite_name_address: { from: ['zz_name', 'zz_addr'] } } },
    wanted: ['zz_name', 'zz_addr'] },
  { name: 'a split PARENT is a member and its prose sibling is not',
    map: { form: 'zz', split: { parent: { parts: ['T1', 'T2'] }, _why: 'prose', bare: { note: 'no parts' } } },
    wanted: ['parent'] },
  { name: 'a declared subject route replaces the printed key with the three the engine reads',
    map: { form: 'zz', map: { zz_tp: 'T' }, subject_classes: { Tp: { class: 'dependent', route: { individual: 'zz_i', entity: 'zz_e', discriminator: 'zz_d' } } } },
    wanted: ['zz_i', 'zz_e', 'zz_d'] },
  { name: 'a dependent class with an INCOMPLETE route is REFUSED',
    map: { form: 'zz', map: { zz_tp: 'T' }, subject_classes: { Tp: { class: 'dependent', route: { individual: 'zz_i' } } } },
    wantedProblem: 'does not declare all of individual, entity and discriminator' },
  { name: 'a route whose printed key is NOT in the key space is REFUSED rather than silently adding three',
    map: { form: 'zz', subject_classes: { Tp: { class: 'dependent', route: { individual: 'zz_i', entity: 'zz_e', discriminator: 'zz_d' } } } },
    wantedProblem: 'is not in the key space' },
