...
export const VACUOUS = [
  ...
  { id: 'G-94', file: '../hubspot/gen-fields-from-map.mjs', anchor: 'const m = /^(S?\\d+[a-z]?)_/i.exec(key);', verdict: 'sound',
    why: 'NO MATCH IS THE ANSWER, NOT A FAILURE TO ANSWER. lineRef returns null for a key naming a concept the form does not number — marital_status, the allowable inputs — and null is written into the output as line_ref: null rather than dropped. A reviewer sees "this key carries no printed marker", which is a fact about the key, and no empty input makes this site report success. THAT MUCH IS THIS SITE\'S OWN AND IT STANDS. What used to follow it did not: this entry claimed a dead regex "is caught one level out: adapters/hubspot/validate-crosswalk.mjs compares the generated file against the crosswalk, whose bindings carry the printed markers independently". VALIDATE-CROSSWALK.MJS DOES NOT READ line_ref, ON ANY FORM. It opens crosswalk.<form>.json, the map and the backbone field file, and it joins them by `key` and `hs_name` — "line_ref" does not occur in it. It also never ran on 433-A at all until [D-16], the very form gen-fields-from-map.mjs writes, because 433-A has no crosswalk: it IS the backbone. So the backstop was named for a form the guard could not be pointed at, checking a column it never reads. Found while closing [D-16]; it is the same shape as [S-21], where 269 claim sites were disposed of on this file\'s coverage of an artefact it had never opened. WHAT ACTUALLY BACKS THE REGEX TODAY: nothing asserts it, and [D-17] carries that rather than a second borrowed sentence. adapters/pdf/regex-self-assert.mjs leaves this file unadopted under a STATED ground — a wrong verdict changes a generated property type, which assert-intake-keys.mjs and generator-guard.mjs compare against the form engine on every run — and that ground covers the TYPE regexes on this file, not line_ref, which is a display column no downstream check reads.' },
