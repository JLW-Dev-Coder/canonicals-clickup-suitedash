...
export const VACUOUS = [
  ...
  // reading of the same packet — which is the [G-182] shape and is sound in the same way, EXCEPT
  // that both readings go to zero together on an input neither can read. That was live when the
  // tool first swept, and the fix is [G-276] below.

  { id: 'G-276', file: 'xfa-fieldset.mjs', anchor: "const rawFieldOpenings = (template.text.match(/<field\\b/g) || []).length;", verdict: 'fixed',
    why: 'WAS VACUOUS AND THE SWEEP IS WHAT SAID SO. This is the second of two readings of one packet — the tag walk produces a field set, this counts `<field` openings in the same bytes, and a walk that lost a subtree makes the two disagree and STOPS. That is [G-182]’s disposition and it holds in the loud direction. What it did NOT cover is the direction where both readings go to zero TOGETHER: an empty template packet, bytes that inflated to something that is not XFA, or a `<field` spelling the walk does not know. The two then AGREE at zero, the artefact writes `fields: 0`, and `--check` reports "regenerates byte-identical" over a form whose field set nothing read. TWO READINGS THAT GO TO ZERO TOGETHER ARE NOT AGREEMENT, THEY ARE THE SAME BLINDNESS TWICE. A zero field count is now a STOP in build(), naming the packet and its byte length, which is [R-04] — zero examined is not a pass — and it is the fifth level of that shape this engine has met.' },

  { id: 'G-277', file: 'xfa-fieldset.mjs', anchor: 'while ((m = re.exec(xml))) {', verdict: 'sound',
    why: 'THE TAG WALK ITSELF, AND IT IS SOUND ONLY BECAUSE OF [G-276]. An xml string this loop matches nothing in produces an empty tree, and extractFields then returns an empty array. That is the vacuous direction, and it does not end in a success message: the zero is now refused by name in build() before any artefact is written. In the other direction the loop cannot silently UNDER-read, because the raw `<field` count is taken from the same string without the walk and the two must agree exactly. The disposition is one object with [G-276] and neither is sound without the other.' },

  { id: 'G-278', file: 'xfa-fieldset.mjs', anchor: 'while ((a = ar.exec(attrStr))) attrs[a[1]] = a[2];', verdict: 'sound',
    why: 'AN EXTRACTION WHOSE EMPTY ANSWER IS A REAL AND COMMON ANSWER, and it cannot reach a success sentence. Most XFA elements carry attributes and some carry none; a field with no `name` attribute is recorded as `(unnamed)` and COUNTED, rather than skipped, so an attribute reading that had gone dead would put every field under `(unnamed)` and collapse 331 distinct SOM paths into one repeated string. That is not quiet: `--check` compares the whole artefact byte for byte against the committed bytes, so a walk that lost every name differs on every row and STOPS. The empty-attrStr case is therefore disposed as a real state rather than as a blind spot.' },

  { id: 'G-279', file: 'xfa-fieldset.mjs', anchor: 'const m = /<dynamicRender[^>]*>([^<]*)</.exec(cfg.text);', verdict: 'sound',
    why: 'THE ABSENCE IS RECORDED RATHER THAN DEFAULTED, which is the whole disposition. `dynamicRender` is the form declaring whether a viewer may render it statically, and a config packet that does not declare one is a real state on real forms. A null match here does NOT fall back to a value: it writes the string "(the config packet declares no dynamicRender)" into the artefact, and a missing config packet writes "(no config packet)". Both are visible in the committed bytes and both differ from every legal value, so a reading that went dead reports as an absence somebody can see rather than as `forbidden` — which is the answer that would have made this form look like the five that fill.' },

  { id: 'G-280', file: 'xfa-fieldset.mjs', anchor: '} catch (e) {', verdict: 'sound',
    why: 'A CATCH THAT PUSHES A PROBLEM AND CANNOT REACH A PASS. A packet declared FlateDecode that will not inflate is pushed into `problems` with the packet name and the zlib message, and the CLI exits 2 on any non-empty `problems` before it writes or compares anything. The shape this sweep exists to refuse is the catch that swallows — an unreadable input becoming an empty one and then an agreeing one — and it is refused here by construction: the packet is skipped from the list AND the run is failed, rather than skipped and the run continued. An unreadable packet is a STOP, not an empty packet.' },

  { id: 'G-283', file: 'xfa-fieldset.mjs', anchor: 'return { ok: results.every((r) => r.ok), results };', verdict: 'fixed',
    why: 'WAS VACUOUS, AND IT WAS THE VACUOUS SHAPE INSIDE THE INSTRUMENT WRITTEN TO STOP THE VACUOUS SHAPE. `results.every(...)` is TRUE OF AN EMPTY LIST, so a canary whose cases had been removed, short-circuited or never reached would report "holds" over nothing — and the CLI line printed `${c.results.length} of ${c.results.length}`, which renders that as "0 of 0" and reads like a pass. The tool then goes on to write or certify an artefact. CANARY_CASES is now DECLARED beside the cases and asserted against what actually ran, before `every` is asked; a short list appends its own failure and takes the run down, and the printed line names the declared figure rather than the observed one twice. It is the construct success-sweep.mjs already carries for the same reason, and [R-12] is why it was worth expecting here: the fix for a vacuity is one of the likelier places to write a new one.' },

  { id: 'G-281', file: 'xfa-fieldset.mjs', anchor: "expect('the planted <draw> is not read as a field', !got.some((f) => f.name === 'zz_drawing'), got.map((f) => f.name).join(','));", verdict: 'sound',
    why: 'A NEGATIVE ASSERTION, WHICH IS VACUOUS ALONE AND IS NOT ALONE. `!got.some(...)` is TRUE of an empty field set, so a walker that found nothing would satisfy this case — and that is exactly why the case above it asserts an EXACT COUNT of four rather than a non-empty one. On an empty `got` the count case fails and takes the canary down before this one is reached in any report. The two are ONE DISPOSITION and neither is sound without the other: the count case alone cannot tell a walker that counted the draw from one that dropped a real field, and this case alone cannot tell a working walker from a dead one. Both directions are needed because this walker can fail in both.' },

  { id: 'G-282', file: 'xfa-fieldset.mjs', anchor: "const raw = (CANARY_TEMPLATE.match(/<field\\b/g) || []).length;", verdict: 'sound',
    why: 'A REQUIRED-NON-ZERO ROW INSIDE THE CANARY, so the dead direction is the loud one. The synthetic template plants four fields and this counts them by the same `<field` pattern build() uses on the real packet; the case asserts `raw === got.length`, and a pattern that stopped matching reports 0 against a walk of 4 and takes the canary down. There is no empty input that makes this report success — CANARY_TEMPLATE is a constant in this file and is not read from anywhere. It is the CONTROL FOR [G-276]: the zero-is-a-STOP guard and the walk-versus-raw comparison are only evidence about the real packet if the same pattern is known to agree with the walk on an input where the answer is known, and this is that input.' },
  ...
  // ─── the residual and the four entries its first run forced — [D-24] ──────────────────

  { id: 'G-261', file: 'sweep-boundary.mjs', anchor: "const m = /^([0-9a-z]+)[.](?:pairs[.](?:json|txt)|lineage[.]json)$/.exec(p.split('/').pop());", verdict: 'sound',
    why: "NO MATCH MEANS THIS FILE IS NOT A HYPHENLESS INTAKE RECORD and the filter drops it — but a DEAD pattern here can no longer go quiet, and that is the whole difference between this line and the [SB-24] draft it replaced. If it matched nothing, [SB-24] would claim zero files, and every file it should have claimed would land in [SB-92]'s residual as UNCLAIMED and stop the run by name. The two are wired the opposite way round from the usual hazard: this guard reading nothing makes a DIFFERENT check fail loudly rather than making this one pass. That is the property the residual was built to add." },

  { id: 'G-262', file: 'sweep-boundary.mjs', anchor: "try { return !!declaredGenerator(JSON.parse(r(p))); } catch { return false; }", verdict: 'sound',
    why: "AN UNPARSEABLE MAP ARTEFACT IS NOT A DERIVED CONSTRUCT, and returning false is the safe direction rather than the quiet one. [SB-23] claims a file on the ground that it DECLARES a generator; a file nobody can parse has declared nothing, so it is excluded from that entry — and being excluded from every entry is exactly what [SB-92] refuses. It lands in the residual as UNCLAIMED and names itself. There is no path on which a corrupt sidecar is silently excused as a construct. RE-ANCHORED, and the disposition is unchanged because the BRANCH is unchanged: only the declaration read moved, into declaredGenerator(), so that the population and the crosscheck could not disagree about what counts as a declaration. [R-12] is why the orphan was expected rather than surprising." },
  { id: 'G-308', file: 'assert-runner-derivation.mjs', anchor: "const tokens = [...new Set([...l.text.matchAll(RX_PROBE)].map((m) => m[1]))];", verdict: 'sound',
    why: "A LINE MATCHING NO PROBE CONTRIBUTES NONE, WHICH IS THE COMMON AND CORRECT CASE — most lines in the runner read no path at all, and `continue` on the next line is what makes the scan a scan rather than a report on every line. The dangerous direction is RX_PROBE dying ACROSS THE BOARD rather than on one line, and that direction cannot reach a pass here. Three things stop it, and each stops it on its own. (a) [R-17]: RX_PROBE is registered with adapters/pdf/regex-self-assert.mjs and asserts its own four matches, four rejects and two captures when this module loads, so a source that reached disk with its backslashes eaten throws before the sweep runs. (b) THE CANARY, whose \"an unregistered probe is found\" case plants a probe and requires exactly one finding — a dead pattern yields zero there and exits 5 without touching the runner. (c) THE ORPHAN PASS: five register entries are `kind: probe` and each must match a line, so a pattern finding nothing leaves all five anchors unmatched and reports five ORPHANED ANCHOR problems. The vacuous direction of this site cannot produce a clean run." },
  { id: 'G-309', file: 'assert-runner-derivation.mjs', anchor: "return { results, ok: results.every((r) => r.got === r.expect) };", verdict: 'sound',
    why: "`every` ON AN EMPTY LIST IS VACUOUSLY TRUE AND THE LIST CANNOT BE EMPTY: `results` is mapped one-for-one from CANARY_CASES, which is a literal in this file holding four entries, so its length is fixed at authoring time and is not read from anywhere. AND THE COUNT IS PRINTED BESIDE THE VERDICT rather than left implicit — the CLI reports `N of M case(s) reached the asserted yield`, so a canary list that somehow went empty would print `0 of 0` in the transcript instead of passing silently on a vacuous `every`. That is [R-04] applied to the canary itself: a canary that examined nothing is not a canary that held." },
  { id: 'G-310', file: 'assert-runner-derivation.mjs', anchor: "catch (e) {", verdict: 'sound',
    why: "AN UNREADABLE package.json IS A STOP HERE, NOT AN EMPTY DEFAULT, AND THAT IS THE WHOLE POINT OF THE SITE. This sweep derives its POPULATION from package.json — [R-22], pre-flight discovers and is never told a path — so a manifest that will not parse means there is no subject, and a sweep of no subject reports zero problems and reads exactly like a clean runner. It is the one catch in this file that exits rather than continuing, and the message says which of the two it is. It is deliberately NOT the empty-string treatment [G-243] and [G-257] give the same read in blanket-audit.mjs and sweep-boundary.mjs: there the manifest is one input among many and a bad parse should fail its own row, whereas here it is the only input and failing quietly would certify the file this sweep exists to judge." },
  { id: 'G-311', file: '../hubspot/check-cf-mirror-export.mjs', anchor: "catch (e) { console.error(`STOP — ${OUT}.json will not parse: ${e.message}`); process.exit(3); }", verdict: 'sound',
    why: "AN UNREADABLE EXPORT IS A STOP, NOT AN EMPTY DEFAULT, AND THE CATCH IS WHERE IT IS SAID. The JSON is the ONLY input this checker has: every one of its eleven checks is a fact about that file or about a rendering derived from it, so a parse failure means there is nothing to check, and a checker that reported a clean run over nothing is precisely the shape guard-sweep exists to refuse. It exits 3 rather than continuing, and 3 is distinct from the 2 a real disagreement exits with, so a caller can tell \"the artefact is broken\" from \"the artefact disagrees with itself\". The three other reads beside it — .md, .csv and .sha256 — cannot reach a vacuous pass either: an absent one is caught by the existsSync loop above, which exits 3 by name before any comparison runs." },
  { id: 'G-312', file: '../hubspot/check-cf-mirror-export.mjs', anchor: "if (!missing.length && !extra.length) say(1, `header carries exactly the ${HEADER_FIELDS.length} declared field(s)`);", verdict: 'sound',
    why: "BOTH LISTS EMPTY IS SET EQUALITY, NOT AN UNREAD INPUT, AND THIS LINE REPORTS RATHER THAN DECIDES. `want` is HEADER_FIELDS, a literal in this file holding seven names, so it is never empty; `missing` is what want has and got does not, and `extra` the reverse. A header read as {} yields missing = 7 and the line does not fire. A header carrying the seven plus one more yields extra = 1 and it does not fire. The only way both are empty is that the key sets are equal, which is the claim. AND THE VERDICT DOES NOT LIVE HERE: say() appends to the ok[] transcript and nothing else — the two problems.push() calls immediately above are what fail the run, and they fire on exactly the complement of this condition. A vacuous pass would have to make both of those not fire, which is the same set equality." },
  { id: 'G-313', file: '../hubspot/check-cf-mirror-export.mjs', anchor: "if (!tagDiff.length && tagSum === h.record_count)", verdict: 'sound',
    why: "THE SECOND CONJUNCT IS WHAT STOPS THE VACUOUS DIRECTION, AND IT IS THERE FOR THAT REASON. `tagDiff` alone could go empty by the population going empty: tagKeys is the union of the header’s tag_counts keys and the tags counted off the records, so a header with no tag_counts AND records carrying no prefix_tag would produce an empty union and an empty diff. `tagSum === h.record_count` refuses exactly that case — an empty tag_counts sums to 0, and record_count cannot be 0 because an empty records array is a STOP at the top of this file, before any check runs ([R-04]). So the two conditions together say the partition is disjoint AND total, and neither can be satisfied by an input nobody read. As with [G-312] this line only appends to the transcript; the two problems.push() calls above it are the verdict." },
  { id: 'G-299', file: 'sweep-boundary.mjs', anchor: "try { scripts = JSON.stringify(JSON.parse(r('package.json')).scripts || {}); } catch { scripts = ''; }", verdict: 'sound',
    why: "THE EMPTY STRING IS THE LOUD DIRECTION, AND IT IS [G-243]'S GROUND ON THE NEIGHBOURING ENTRY. [SB-29] excuses the mirror-export renderings on the ground that a standing script re-derives them, and this reads package.json to check that. An unreadable package.json yields the empty string, `scripts.includes(checker)` is then false, and the entry is reported CONTRADICTED -- 'nothing re-derives the renderings, so every one of them is an unchecked artefact' -- and the run exits non-zero. There is no arrangement of this catch returning less that produces a clean sweep. It is deliberately not a throw, for the same reason as [G-243]: a boundary register whose report depends on one file parsing should say what it could not read in the row that depends on it, rather than take down every other entry in the same run." },
  { id: 'G-300', file: 'assert-name-sets-imported.mjs', anchor: "const cyOK = cy.decls.length === 1", verdict: 'sound',
    why: "THE `every` IS GUARDED BY AN EXACT LENGTH EQUALITY IN FRONT OF IT, and that is the whole construct. `cyKeys.every(...)` is vacuously true on an empty hit list, so a canary whose detector had gone blind would satisfy it and certify the sweep it exists to guard -- the canary passing over nothing, which is the shape [R-04] names and which this engine has now met at six levels. CANARY_EXPECT is DECLARED beside the planted files and `cyKeys.length === CANARY_EXPECT.length` is asserted BEFORE `every` is reached, so a detector returning zero hits fails on the length rather than passing on the emptiness. The declaration count is asserted the same way in the same expression: exactly one, and it must be named FOO_SET, so a reader that found nothing and a reader that found everything are both refused. On failure the run exits 5 WITHOUT TOUCHING THE TREE, because a sweep whose canary did not come back must not report on the real population at all." },
  { id: 'G-291', file: 'assert-name-sets-imported.mjs', anchor: "try { names = readdirSync(root); } catch { continue; }", verdict: 'sound',
    why: "AN UNREADABLE ROOT SKIPS FILES, AND SKIPPED FILES CANNOT GO QUIET. A root that will not list contributes no sources, so any name set only that root retyped stops being found. The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards. Continuing rather than throwing is deliberate: adapters/clickup did not exist for most of this repo's life and a root that is legitimately absent should not take down a sweep that has three others to read." },
  { id: 'G-292', file: 'assert-name-sets-imported.mjs', anchor: "try { if (statSync(p).isDirectory()) continue; } catch { continue; }", verdict: 'sound',
    why: "A PATH THAT WILL NOT STAT IS NOT A FILE THIS SWEEP CAN READ, and dropping it has the same consequence as an unreadable root: fewer sources, fewer hits, stale baseline, non-zero exit. The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards." },
  { id: 'G-293', file: 'assert-name-sets-imported.mjs', anchor: "for (const m of s.matchAll(re)) {", verdict: 'sound',
    why: "A FILE MATCHING NO DECLARATION CONTRIBUTES NONE, WHICH IS THE COMMON AND CORRECT CASE — most files in this tree export no name set at all. The dangerous direction is the regex dying across the board rather than per file, and that is caught twice over: [R-04]'s explicit STOP fires when decls is empty, and The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards." },
  { id: 'G-294', file: 'assert-name-sets-imported.mjs', anchor: "const members = [...m[2].matchAll(/'([^']+)'|\"([^\"]+)\"/g)].map(x => x[1] ?? x[2]);", verdict: 'sound',
    why: "AN EMPTY MEMBER LIST IS DISCARDED TWO LINES BELOW, by the length < 2 test, because a set of nothing is not a vocabulary and asserting over it would manufacture a finding out of an unread declaration. A member reader that died everywhere empties decls entirely, and The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards." },
  { id: 'G-295', file: 'assert-name-sets-imported.mjs', anchor: "const allPresent = d.members.every(x => s.includes(`'${x}'`) || s.includes(`\"${x}\"`));", verdict: 'sound',
    why: "`every` ON AN EMPTY MEMBER LIST IS VACUOUSLY TRUE AND CANNOT BE REACHED, because a declaration with fewer than two members is discarded before it becomes a d. This is a PRE-FILTER, not the verdict: a true result only advances to the set-equality test below, which is what actually decides a hit. The vacuous direction here widens the candidate set and then fails the real test, rather than producing a finding." },
  { id: 'G-296', file: 'assert-name-sets-imported.mjs', anchor: "const bracketed = [...s.matchAll(/\\[([^\\]]*)\\]/g)].some((b) => {", verdict: 'sound',
    why: "`some` ON NO BRACKETS IS FALSE, WHICH IS THE SAFE DIRECTION: a file containing no array literal cannot be retyping a list, and the site is skipped. The failure mode worth naming is the opposite one — this regex matching everything and reporting phantom sites — and that direction is loud by construction, because a NEW hit fails the run and has to be read. The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards." },
  { id: 'G-297', file: 'assert-name-sets-imported.mjs', anchor: "const got = new Set([...b[1].matchAll(/'([^']+)'|\"([^\"]+)\"/g)].map((x) => x[1] ?? x[2]));", verdict: 'sound',
    why: "AN EMPTY got FAILS THE SIZE TEST ON THE NEXT LINE, because want is never empty — a declaration with fewer than two members never became a d. So a dead member reader here produces size 0 against size >= 2, no hit, and the baseline then goes stale. The BASELINE below asserts its 24 carried sites in BOTH directions, so a reader that goes blind here yields zero hits and makes all 24 stale at once, which exits non-zero. The vacuous direction cannot reach a pass, and [R-04]\\u2019s zero-examined STOP beside it names the cause rather than leaving 24 stale rows to be read backwards." },
  { id: 'G-298', file: 'assert-name-sets-imported.mjs', anchor: "return got.size === want.size && [...want].every((x) => got.has(x));", verdict: 'sound',
    why: "THE `every` IS GUARDED BY THE SIZE EQUALITY IN FRONT OF IT, and want is never empty, so the vacuously-true case cannot be reached. SET EQUALITY IS THE POINT AND IT IS THE SECOND DRAFT: the first used containment, under which a longer list that merely included the members counted as a retyping, and this file reported ITSELF because its four-element ROOTS contains every member of a two-element DIRS. Equality is what \"retyped\" means; the superset case is declared a blind spot in the header rather than counted." },
  { id: 'G-290', file: 'sweep-boundary.mjs', anchor: "} catch (e) { out.push('[SB-28] adapters/clickup/write-probe.json will not parse", verdict: 'sound',
    why: "THE CATCH PUSHES A PROBLEM, WHICH IS THE LOUD DIRECTION AND THE ONLY ONE AVAILABLE HERE. [SB-28] claims adapters/clickup on a ground that includes 'every ClickUp write probe this repo made was torn down', and that ground is checked by reading the register. An unreadable or unparseable register does NOT mean no probe was left live — it means nobody can tell, and those are different facts. So the catch reports a problem naming the file rather than returning an empty list, and the entry is CONTRADICTED until the register can be read. The vacuous shape is the one NOT taken: a catch that returned nothing would let a corrupted register read as 'no probes outstanding', which is a synthetic task left on a target this repo does not own, reported clean. It is deliberately not a throw, for [G-243]'s reason on the neighbouring entry: a register whose report depends on one file parsing should say what it could not read in the row that depends on it, rather than take down every other entry in the same run." },
  ...
  { id: 'G-263', file: 'sweep-boundary.mjs', anchor: "try { n = e.count(); } catch { n = null; }", verdict: 'sound',
    why: "A count() THAT THROWS IS NOT A COUNT THAT AGREES. n stays null, the `typeof n === \"number\"` test below is false, and no drift row is emitted for that entry — which looks like a silence and is not one, because the runner evaluates the SAME count() for the row it prints and pushes an UNREADABLE problem naming the boundary when it throws ([G-111]). The entry cannot pass: it is reported by the runner and merely not double-reported here. Comparing a thrown count against a real claims() length is the alternative, and it would manufacture a drift row out of an unreadable input." },

  { id: 'G-264', file: 'sweep-boundary.mjs', anchor: "const tops = roots.filter((d) => !roots.some((o) => o !== d && d.startsWith(o + '/')));", verdict: 'sound', family: true,
    why: "THIS NARROWS THE ROOTS AND CANNOT WIDEN THE RESIDUAL'S BLIND SPOT. git ls-files is recursive, so a nested root would list the same files a second time; dropping it changes nothing about WHICH files are enumerated. If the `some` found nothing, every root would be kept, git would return duplicates and the residual would be computed over a superset of the truth — more files examined, not fewer. The failure direction of an empty read here is a noisier check, never a quieter one, and [SB-92]'s canary passes an explicit root list so its verdicts do not depend on this line at all." },

  { id: 'G-265', file: 'sweep-boundary.mjs', anchor: "for (let i = 0; i < ids.length && allDeclared; i++)", verdict: 'sound', family: true,
    why: "THE LOOP IS ENTERED ONLY WHERE ids.length >= 2, which the line above it establishes with an explicit `if (ids.length < 2) continue`. So `ids.length &&` is not a guard against an empty set here; it is the early exit of a pair scan that has already found an undeclared pair. An empty ids array cannot reach this line, and a single-claimant file is not an overlap. The state this would hide — a file claimed twice, reported as claimed once — requires ids.length >= 2, at which point the loop runs." },

  { id: 'G-266', file: 'sweep-boundary.mjs', anchor: "for (let j = i + 1; j < ids.length && allDeclared; j++)", verdict: 'sound', family: true,
    why: "The inner half of the same pair scan, and the same disposition: unreachable with fewer than two claimants, and the `allDeclared` conjunct is a short-circuit after a verdict has been reached rather than a test of whether there was anything to look at. Disposed separately from [G-265] rather than as one family, because the two carry different bounds — i from 0, j from i+1 — and a family disposition would certify a bound it had not read." },

  { id: 'G-267', file: 'sweep-boundary.mjs', anchor: "try { doc = JSON.parse(r(p)); } catch (e) { out.push(`[SB-25] UNREADABLE — ${p}: ${e.message}`); continue; }", verdict: 'sound',
    why: "THE CATCH PUSHES A PROBLEM NAMING THE FILE AND THE PARSE ERROR, then continues to the next register. [SB-25] excuses a cross-form register on the claim that the count it states about itself is derived HERE from its own contents; a register nobody could open has had no count derived, and the run says so by name and exits non-zero. An empty out[] from this loop means every register parsed and every _count agreed — the two are not confusable." },

  { id: 'G-268', file: 'sweep-boundary.mjs', anchor: "let doc; try { doc = JSON.parse(r(p)); } catch { return `[SB-25] ${p} — UNREADABLE`; }", verdict: 'sound',
    why: "THE OBSERVE HALF, AND IT RETURNS THE WORD UNREADABLE RATHER THAN A ROW THAT LOOKS LIKE A READING. observe() is reporting rather than judging — the crosscheck above is what stops the run, and [G-267] disposes it — so the requirement here is only that an unopenable file never appears in the transcript as though its counts had been read. It appears as UNREADABLE, beside a crosscheck problem naming the same file." },

  { id: 'G-269', file: 'sweep-boundary.mjs', anchor: "const m = /^([0-9a-z]+)[.](pairs|lineage)[.]/.exec(f);", verdict: 'sound',
    why: "NO MATCH IS A STOP HERE AND NOT A SKIP, which is what separates it from [G-108] and from every other extract in this file. The file being parsed was selected by SB24_FILES()'s own pattern moments earlier, so a failure to parse it back into a form and a kind means the SELECTOR and the CHECK disagree about what this entry stands over — and the branch pushes exactly that sentence as a problem. `continue` there would have meant the more files the selector took that the checker could not read, the fewer successors were demanded. A dead regex makes every file report UNREADABLE; it cannot make one report checked." },

  { id: 'G-270', file: 'sweep-boundary.mjs', anchor: "const m = /^([0-9a-fA-F]{64})\\s+\\*?(\\S+)\\s*$/.exec(line.trim());", verdict: 'sound',
    why: "A NON-BLANK LINE THAT DOES NOT PARSE IS PUSHED AS UNREADABLE, naming the line, and a blank one is skipped — the two are separated by the `line.trim() &&` in the same statement, so an unreadable pin file cannot be read as an empty one. AND THE DEAD-PATTERN CASE IS CLOSED IMMEDIATELY BELOW: if the regex matched nothing, `pinned.size` would be 0 and the next line refuses the entry in as many words — \"a checksum list over an empty set proves nothing, and every matches below would be vacuous\". That refusal exists because this file spent its whole life until this commit being read by nothing at all, and a reader that silently parses zero records would restore exactly that state under a green line." },

  { id: 'G-271', file: 'sweep-boundary.mjs', anchor: '} catch (e) { gitUnreadable = e.message; }', verdict: 'sound',
    why: "GIT FAILING IS A THIRD STATE AND IT IS CARRIED AS ONE. `gitUnreadable` holds the message, and [SB-92]'s crosscheck reads it FIRST and returns before it looks at the residual: \"a residual whose universe cannot be read reports that it could not be read. Never a pass.\" Without that early return, a machine without git would produce an empty universe, an empty residual and the strongest possible pass over a tree nothing had been enumerated in - the exact substitution [G-112] disposes one construct over, and the reason the empty-universe case is ALSO refused explicitly on the line below it." },

  // ─── the saturation union over a form's declared subject set — [DM-2] ─────────────────

  { id: 'G-272', file: 'saturation-union.mjs', anchor: "try { rec = JSON.parse(r(path)); } catch (e) { return { unreadable: e.message }; }", verdict: 'sound',
    why: "AN UNREADABLE MEMBER RETURNS `{ unreadable }` AND NEVER `{ subject: null }`, which is the whole of the difference. The caller tests `s.unreadable` FIRST and pushes a problem naming the file and the parse error; only then does it look at the subject. Had the catch returned a subject-less record, the member would have failed the \"exactly one member per declared subject\" test as a MISSING SUBJECT — a true report of the wrong cause, pointing an author at the map when the defect is a corrupt fixture. And unionMembers() treats every problem as a STOP rather than as a smaller union, so a member nobody could read can never shrink the set the residual is computed over." },

  { id: 'G-273', file: 'saturation-union.mjs', anchor: "runs.every((x, i) => emptyPer[i].has(cell) || requiredPer[i].has(cell))).sort();", verdict: 'sound', family: true,
    why: "EVERY OVER AN EMPTY runs[] IS TRUE, AND IT CANNOT BE REACHED WITH ONE. The set being filtered is built from those same runs — `everyCellEmptySomewhere` is the union of their empty and required-empty sets — so zero runs means zero candidate cells and the filter walks nothing: the vacuous-true is applied to no element and the result is empty either way. What makes the empty-runs case impossible rather than merely harmless is upstream: unionMembers() always returns the acceptance member or a problem, and a problem returns before this line. The direction that WOULD matter, `every` returning true too easily, is the direction that reports MORE cells unreached and stops the run." },

  { id: 'G-274', file: 'saturation-union.mjs', anchor: "return [...all].filter((c) => rs.every((x, i) => emptyPer[i].has(c) || reqPer[i].has(c))).sort();", verdict: 'sound', family: true,
    why: "THE CANARY'S COPY OF THE SAME EXPRESSION, and it is disposed separately rather than as a family with [G-273] because a family disposition here would let the canary and the live path drift into being different arithmetic under one verdict — which is the [D-11] shape, a check formatting its operand differently from the thing it checks. Both directions of this line are asserted by the canary itself: the planted cell must come back BY NAME and the conforming pair must come back empty, so an `every` that answered true for everything fails the second and one that answered false for everything fails the first." },

  { id: 'G-275', file: 'saturation-union.mjs', anchor: "if (s.runs.length && !s.runs.some((x) => x.failed)) {", verdict: 'sound',
    why: "THIS GATES A TRANSCRIPT BLOCK, NOT A VERDICT. Zero runs means the resolution stopped before any member was filled, which has already pushed a problem, and reportSaturationUnion prints \"SATURATION UNION — n problem(s)\" and returns a non-zero count regardless of this line. The `runs.length &&` exists so a run that never filled anything does not print a table of zeroes that reads like a measurement — the opposite of the hazard this sweep is named for, which is a report of success produced by an empty read. There is no branch here on which an empty runs[] contributes a pass." },
  ...
  { id: 'G-59', file: 'success-sweep.mjs', anchor: '|| lines[i].match(/^\\s*if\\s*\\((.+?)\\)\\s*(?:process\\.exit|(?:^|[^\\w.])stop\\s*\\(|return|STOP|throw|console\\.error)/);', verdict: 'sound',
    why: 'The single-statement `if (x) process.exit(2);` form of the same head. Same disposition as [G-58]: the alternative to matching is not matching, and not matching withholds the `terminal` witness. THE ANCHOR MOVED, AND WHAT MOVED IT IS WORTH RECORDING RATHER THAN RE-ANCHORING SILENTLY. [D-20] replaced all 74 process.exit() call sites under adapters/hubspot/ with stop(), and hs-verify-provision.mjs\'s `if (missing.length || mismatched.length) stop(3);` — a braceless guard, unchanged but for the spelling — stopped matching this alternation, so the terminal sentence below it reported as having NO failure accumulation above it at all. The `stop` clause is what this re-anchoring adds. The orphan check is what required the disposition to be re-read instead of the anchor being quietly patched, which is the whole reason an anchor is a string and not a line number.' },
  ...
  // ─── assert-reachability.mjs — the reachability graph and its canary ───────────────────
  //
  // TEN SITES, AND THE HONEST ANSWER FOR MOST OF THEM IS THAT AN EMPTY EXTRACTION MAKES THIS
  // TOOL REPORT MORE, NOT LESS. Its verdict is over the tools it CANNOT reach: an edge reader
  // that stopped finding edges would shrink the reachable set and turn 65 run tools into 65
  // UNRUN AND UNREGISTERED problems. That is the loud direction, and it is loud by construction
  // rather than by luck, because the register standing beside the graph is the OTHER direction —
  // an excuse over a tool the suite does run is equally a STOP. Neither an empty graph nor a
  // total one reports clean.

  { id: 'G-230', file: 'assert-reachability.mjs', anchor: 'while (i < src.length && /\\s/.test(src[i])) i++;', verdict: 'sound',
    why: 'A WHITESPACE SKIP BETWEEN A CALL NAME AND ITS OPENING PARENTHESIS, not a guard. Its empty case is `spawnSync(` with no space, which is how every call site in this tree is actually written, so the loop body running zero times is the NORMAL path and not a degenerate one. What it protects is `spawnSync (x)`; if it stopped advancing, argRegion returns the empty string, the call contributes no edge, and the tool it names becomes UNRUN AND UNREGISTERED — a report, never a silence.' },

  { id: 'G-231', file: 'assert-reachability.mjs', anchor: 'for (const m of masked.matchAll(re)) {', verdict: 'sound', family: true,
    why: 'THE LOCAL-RUNNER SCAN, AND AN EMPTY YIELD LOSES EDGES RATHER THAN GAINING THEM. It finds functions in a file whose own bodies spawn — `runTool` in run-form-gate.mjs is the motivating one. Finding none means the gate contributes no spawn edges at all, at which point twelve gate steps, five fill engines and every tool the gate drives come out unreached and the run prints them by name. The direction that would be quiet is the OPPOSITE one — a scan that matched everything, turning every function into a runner and every string into an edge — and that is the direction the graph canary plants at cases (c) and (d), where a tool named in a message string and a tool named in a register must each yield NOTHING.' },

  { id: 'G-232', file: 'assert-reachability.mjs', anchor: "for (const m of keepStrings(src.get(p)).matchAll(/(?:from\\s*|import\\(\\s*)['\"](\\.[^'\"]+\\.mjs)['\"]/g)) {", verdict: 'sound',
    why: 'THE IMPORT-EDGE READER, and the same shape as [G-231]. An empty yield means no module is reached by import, so every library that has no CLI guard — and only those are certified by an import — falls out of the reachable set and is reported. It cannot manufacture a reach: it matches a relative specifier ending .mjs and then requires the resolved path to be a file this run actually enumerated, so a specifier it misreads resolves to nothing and is dropped rather than invented.' },

  { id: 'G-233', file: 'assert-reachability.mjs', anchor: 'for (const m of code.matchAll(new RegExp(`\\\\b${fn}\\\\b`, \'g\'))) {', verdict: 'sound',
    why: 'THE SPAWN CALL-SITE SCAN, over the CODE-MASKED text, and the mask is why this is sound rather than merely quiet. Finding no call site means no spawn edges from that file, which shrinks the reachable set and prints names. Finding one that is not a call site is the dangerous direction and is exactly what the mask removes: guard-sweep.mjs\'s own register quotes `spawnSync(process.execPath, [\'-e\'` as the ANCHOR TEXT of [G-15], and reading call sites out of the string-preserving text made that anchor a spawn call whose argument region then ran on through the rest of this register and turned every tool it names into an edge. That is how correlate-labels.mjs came out reachable. The call site is found in the masked text and the argument region is read from the unmasked text at the same index.' },

  { id: 'G-234', file: 'assert-reachability.mjs', anchor: 'for (const nm of region.matchAll(NAME_IN_STRING))', verdict: 'sound',
    why: 'THE NAMES INSIDE ONE SPAWN CALL\'S ARGUMENT REGION. An argument region with no .mjs name in it is ordinary — `spawnSync(process.execPath, [\'-e\', script])` builds its child from a string — and yields nothing, which costs an edge and never invents one. Every name it does yield is then required to resolve to a file this run enumerated, so a misread name is dropped rather than added.' },

  { id: 'G-235', file: 'assert-reachability.mjs', anchor: "try { gen = JSON.parse(readFileSync(defs, 'utf8'))?.meta?.generator ?? null; } catch { gen = null; }", verdict: 'sound',
    why: 'AN UNREADABLE DEFINITIONS FILE YIELDS NO EDGE, WHICH IS THE LOUD DIRECTION HERE. This is the modelled half of rerun-regression.mjs\'s declared discovery: it re-reads each `fields.<form>.json` for the `meta.generator` [R-19] requires it to carry. If the file is missing or will not parse, `gen` is null, that generator gets no spawn edge, and the deriver is reported UNRUN AND UNREGISTERED by name. The same unreadability is a STOP with a message in rerun-regression.mjs itself and in headroom.mjs, so the state is not swallowed anywhere — it is simply not this file\'s finding to make, and here it fails towards reporting.' },

  { id: 'G-236', file: 'assert-reachability.mjs', anchor: "for (const m of String(cmd).matchAll(/adapters\\/(?:pdf|hubspot)\\/[\\w.-]+\\.mjs/g))", verdict: 'sound',
    why: 'THE ROOT SET, READ OUT OF package.json\'S SCRIPTS. An empty yield is the emptiest possible input to this tool and it is the loudest: with no roots the traversal never starts, nothing is reachable, and all 85 tools report as unrun. There is no arrangement of this extraction returning too little that produces a clean run.' },

  { id: 'G-237', file: 'assert-reachability.mjs', anchor: "for (const m of text.matchAll(new RegExp(`\\\\b${fn}\\\\b`, 'g'))) {", verdict: 'sound', family: true,
    why: 'TWO SITES INSIDE THE GRAPH CANARY\'S OWN PROBE, and they are the one place in this file where an empty extraction WOULD be quiet — which is why the probe is judged against fixed expected answers rather than against a count. probe() re-runs the same call-site reader over four synthetic sources whose answers are known: two must yield a named edge and two must yield none. A probe that returned nothing fails cases (a) and (b) immediately; a probe that returned everything fails (c) and (d). Both directions are planted, so neither an empty reader nor a total one can pass, and the six results are printed on every run.' },

  { id: 'G-238', file: 'assert-reachability.mjs', anchor: "ask('f  the form set behind `${form}` is DERIVED, never listed', g.forms.length > 0 && g.forms.every((f) => typeof f === 'string' && f.length > 0), true)", verdict: 'sound',
    why: 'THE `every` IS GUARDED BY THE LENGTH TEST IN FRONT OF IT, AND THAT IS THE WHOLE POINT OF THE CASE. `[].every(...)` is true, so on an empty form list this canary would certify that the form set is derived and non-empty while it was neither — and the form set is what expands the gate\'s `fill-${form}.mjs` into real names, so an empty one silently removes five fill engines from the graph. `g.forms.length > 0 &&` is what makes the empty case FAIL, and the expected answer is asserted as `true` rather than compared against a count, so the case cannot be satisfied by an absence.' },

  { id: 'G-242', file: 'sweep-boundary.mjs', anchor: "const scripts = (() => { try { return JSON.stringify(JSON.parse(r('package.json')).scripts || {}); } catch { return ''; } })();", verdict: 'sound',
    why: 'THE EMPTY STRING IS THE LOUD DIRECTION HERE, NOT THE QUIET ONE. [SB-23] excuses two construct artefacts on the ground that a generator re-derives each of them on a standing run, and this reads package.json to check that the named generator is in one. An unreadable package.json yields the empty string, `scripts.includes(gen)` is then false for every artefact, and EVERY ONE of them is reported CONTRADICTED -- the boundary collapses to "nothing re-derives these" and the run exits non-zero. There is no arrangement of this catch returning less that produces a clean sweep. It is deliberately not a throw: a boundary register whose report depends on one file parsing should say what it could not read in the row that depends on it, rather than take down every other entry in the same run.' },

  { id: 'G-243', file: 'sweep-boundary.mjs', anchor: "try { gen = declaredGenerator(JSON.parse(r(p))); } catch { gen = null; }", verdict: 'sound',
    why: 'null IS A NAMED FAILING VERDICT AND THE NEXT LINE PRINTS IT. An artefact that will not parse, or that parses and names no generator, sets gen to null and the loop immediately pushes CONTRADICTED -- "an artefact that does not say what wrote it cannot be checked against it". The unreadable case and the undeclared case deliberately produce the SAME row, because for this entry\u2019s claim they are the same fact: the ground that something re-derives this file cannot be verified. What is NOT possible is silence, which is what `catch { continue }` would have been. RE-ANCHORED for the declaredGenerator() factoring; the branch and its verdict are unchanged.' },
  { id: 'G-244', file: 'target-root.mjs', anchor: "catch (e) { return { stop: `${p} will not parse: ${e.message}. An unreadable field list is not an absent one.` }; }", verdict: 'sound',
    why: 'IT RETURNS A STOP, AND THE STOP IS THE ONLY THING IT CAN RETURN FROM HERE. The empty input is a field list that will not parse, and the tempting shape -- catch { return {} } or a fallback to the historic literal -- is the whole defect this module exists for: a root that is not this form’s makes every target predicate select nothing while every check built on it reports a pass over the empty set. There is no branch below this that can produce a root. Both callers refuse to continue: adapters/pdf/validate-map.mjs exits 2 naming the form, and adapters/pdf/verify-form-coverage.mjs throws at load if the derivation yields no root at all. The sibling function rootPrefixOf() additionally treats an EMPTY list, a name with no dotted path, and two roots in one list as three separate stops, each planted in the canary with a required STOP.' },
  { id: 'G-245', file: 'fill-433d.mjs', anchor: 'try { field = form.getTextField(name); } catch { skipped.push(name); continue; }', verdict: 'sound',
    why: '[G-169]\u2019S GROUND ON THE MIRRORED ENGINE. A target this form has no text field for is RECORDED in `skipped`, carried into the run\u2019s tripwires file and printed in the closing line, so a map naming a cell the PDF does not draw shows as a non-zero skip count rather than as a quieter success. It cannot happen silently either way: validate-map.mjs resolves all 188 target references in this map against the field list at gate step 3, before this engine is reached. The `continue` rather than `return` is the mirror: this loop writes BOTH copies of one value, so a failure on one copy must not abandon the other -- and if only one copy were written, [M-07] and [M-08] would each stop the run over it.' },

  { id: 'G-246', file: 'fill-433d.mjs', anchor: 'catch (e) { capacityErrors.push({ key, name, len: s.length, max, value: s, why: e.message }); }', verdict: 'sound',
    why: 'A VALUE THE FIELD REFUSES IS A HARD FAILURE, NOT A SKIP -- [G-170]\u2019s ground exactly. The catch records the throw with the input key, the widget, the length and the declared maximum, and `capacityErrors.length` STOPS the run before any PDF is written. It is the path samples/433d.overmax.sample.json exists to drive: 433-D declares no repeatable group, so there is no overflow to log and capacity is the whole of what a stress record can stress.' },

  { id: 'G-247', file: 'fill-433d.mjs', anchor: 'catch { skipped.push(name); }', verdict: 'sound',
    why: 'THE CHECKBOX ARM OF [G-245], with the same downstream guard [G-171] names. A box that cannot be fetched is pushed to `skipped` and never reaches `ticked`, so it is not read back -- and every box that IS ticked is, with the read-back stopping the run on any that did not turn on. All 26 of this form\u2019s checkboxes store /1, which was PROBED rather than assumed; the read-back stands whether they are uniform or not, because a uniform reading today is not a guarantee about the next revision.' },

  { id: 'G-248', file: 'fill-433d.mjs', anchor: 'try { held = form.getTextField(name).getText(); read = true; } catch { /* not a text field */ }', verdict: 'sound',
    why: 'THE CATCH SETS NO VALUE AND CLEARS NO FLAG, AND THAT IS THE WHOLE DESIGN OF THIS BLOCK. It is [SC-7] -- proving that a cell existing for ONE legal person is empty on a record declaring the other -- and the shape that would make it vacuous is `catch { held = \'\' }`, under which a field that could not be read becomes the empty string, compares equal to empty, and SATISFIES the assertion by having read nothing. Here `read` stays false, the next line tries the checkbox accessor, and if THAT also throws the target lands in `unreadable`, which is its own hard failure with its own message: "an unreadable cell is not a cell proved empty". `undefined` is not caught by either arm -- it is what pdf-lib RETURNS for a text field carrying no /V, which is a real reading and is the empty answer this check is looking for.' },

  { id: 'G-249', file: 'fill-433d.mjs', anchor: "if (!read) { try { held = form.getCheckBox(name).isChecked() ? 'checked' : undefined; read = true; } catch { /* nor a checkbox */ } }", verdict: 'sound',
    why: 'THE CHECKBOX ARM OF [G-248] AND THE SECOND OF TWO ATTEMPTS, not a fallback that swallows the first. Two of the five subject-conditional cells on this form ARE checkboxes -- the two review codes -- so this arm is load-bearing rather than defensive: without it the IMF and BMF codes would be unreadable, land in `unreadable`, and stop every run. Failing both arms leaves `read` false and produces a named failure; it never produces a value.' },

  { id: 'G-250', file: 'fill-433d.mjs', anchor: 'const on = targets.filter((t) => { try { return form.getCheckBox(t).isChecked(); } catch { return false; } });', verdict: 'sound',
    why: 'THE EXCLUSIVE ASSERTION, AND `false` IS THE SAFE DIRECTION -- [G-172] verbatim. An unreadable box counts as NOT checked, which can only make a set look MORE exclusive, so this catch cannot manufacture a pass over a real violation, only understate one. On this form there are SIX sets rather than three, one per printed column per copy, and gate step 10 requires EXACTLY ONE checked in each on a saturated record -- which is the stronger claim and is made against the filled document rather than here.' },

  { id: 'G-251', file: 'fill-433d.mjs', anchor: 'const notOn = ticked.filter(', verdict: 'sound',
    why: 'AN EMPTY `ticked` IS AN EMPTY PROBLEM LIST AND THAT IS THE RIGHT ANSWER -- [G-168]\u2019s ground. Every box this engine ticked is read back off the same form object to prove it turned on; if nothing was ticked there is nothing that could have failed to turn on, and the count of boxes ticked is printed on every run whether it is zero or ten. The catch inside the filter returns TRUE, not false: a box that cannot be read back is a finding, not a pass. On this form every tick is a PAIR of ticks, so a copy-specific failure to turn on is caught here before [M-08] would catch it in the saved bytes.' },

  { id: 'G-252', file: 'count-sweep.mjs', anchor: 'derived: (ctx.joinedAllPages.match(which.re) || []).length,', verdict: 'sound',
    why: 'AN EXTRACTION WHOSE REQUIRED ANSWER IS ZERO, disposed the way [G-183] and [G-191] are: it is carried by a POSITIVE CONTROL in its own derivation. [S-50] asserts that 433-D draws no run matching /Add lines?/, /\\bTotals?\\b/ or /minus/ on any page, so each of these three rows compares a claimed 0 against a derived 0 and a pattern that stopped matching would report exactly the zero it is required to report. [S-51] runs the SAME joined page text through the SAME kind of match with a required NON-ZERO answer -- 433-D prints "Agreement" in its own title -- so a dead reading reports 0 against a required non-zero and takes the run down. THE ABSENCE IS ONLY READABLE BECAUSE OF THE CONTROL, and it matters more here than on the forms that print totals: this one\u2019s totals file exists ONLY to state the absence, and a totals file whose zeros could not be trusted would be worse than none.' },

  { id: 'G-253', file: 'count-sweep.mjs', anchor: "claimed: true, derived: (ctx.joinedAllPages.match(/Agreement/gi) || []).length > 0,", verdict: 'sound',
    why: 'THE POSITIVE CONTROL ITSELF, and it is required NON-ZERO so the dead direction is the loud one. 433-D prints "Installment Agreement" in its masthead and the word again in the IRS-use block and in the terms; a regex that stopped matching reports false against a claimed true and stops the run. There is no empty input that makes this row report success, which is the whole reason the three zero rows above it can be believed. The row beside it derives WHICH PAGES draw runs at all -- page 2 draws none -- so a reading that had gone dead on the geometry rather than on the pattern is caught by a different row than the one it would have quietened.' },

  { id: 'G-254', file: 'verify-form-coverage.mjs', anchor: "catch (e) { throw new Error(`verify-form-coverage: ${recordPath} will not parse: ${e.message}", verdict: 'sound',
    why: 'IT THROWS, AND THE FIRST DRAFT DID NOT -- WHICH IS WHY THIS ENTRY EXISTS RATHER THAN THE VERDICT BEING OBVIOUS. That draft returned `{ active: false }` on an unparseable record, on a record carrying no subject, and on a map naming no mirror. Under it, any of those three switched off BOTH halves of the conditional construct at once: the saturation EXEMPTION, whose loss is safe because saturation then fails louder, and the CHECK that a cell required to be empty is empty, whose loss is an entity record carrying a spouse\u2019s signature passing unnoticed. They are not separable by a flag, so neither is optional, and every failure in this function is now a throw with the contradiction named. The record is the one step 7 filled from and step 7 read it successfully, so an unreadable one here is a contradiction rather than a missing input. A form declaring no subject-conditional cell reaches none of this and returns inactive on the first line, which is the five forms that preceded 433-D.' },

  { id: 'G-255', file: 'blanket-audit.mjs', anchor: "const demanded = [...new Set(sites.flatMap((x) => [...String(x.value).matchAll(/\\[(M-\\d{2})\\]/g)].map((m) => m[1])))];", verdict: 'sound',
    why: 'AN EMPTY DEMAND IS ALREADY A NAMED STATE IN THIS FILE AND IT IS A STOP UNLESS DECLARED. That is what makes this extraction safe where the same shape elsewhere would not be: if this regex stopped matching, `demanded` would be empty, `emptyDemand` would be true, and the audit would report "EMPTY DEMAND [S-52] -> assert-mirror.mjs -- the prover extracted NONE of its own atoms from the sites this blanket covers, so it proved the citation over nothing" and exit non-zero, unless an EMPTY_DEMAND entry declared the absence with a reason true of that form. It is not hypothetical: this exact pair reported EMPTY DEMAND on its first run, because the map\u2019s prose named the construct without naming its clause ids, and the citation was made checkable rather than the report silenced.' },

  { id: 'G-256', file: 'blanket-audit.mjs', anchor: "const demanded = [...new Set(sites.flatMap((x) => [...String(x.value).matchAll(/\\[(SC-\\d+)\\]/g)].map((m) => m[1])))];", verdict: 'sound',
    why: '[G-255]\u2019S GROUND ON THE SUBJECT-CLASS PROVER. An empty extraction produces EMPTY DEMAND, which is a STOP unless declared, so a dead regex is loud rather than quiet. The SUPPLIED side is additionally incapable of going silently empty: it is the CLAUSES register imported from adapters/pdf/assert-subject-class.mjs, and adapters/pdf/register-ids.mjs holds that register against every other id-keyed register in the tree and refuses a run in which it contributes nothing.' },

  { id: 'G-257', file: 'blanket-audit.mjs', anchor: "try { scripts = JSON.stringify(JSON.parse(readFileSync('package.json', 'utf8')).scripts || {}); } catch { scripts = ''; }", verdict: 'sound',
    why: 'THE EMPTY STRING IS THE LOUD DIRECTION, which is [G-242]\u2019s disposition of the same read in the boundary register. [K-26] counts the 433-D construct files whose declared generator a standing npm script actually runs; an unreadable package.json yields the empty string, `scripts.includes(gen)` is false for every file, and the counter reports 0 of 3 COVERED -- a completeness claim failing over its whole universe. There is no arrangement of this catch returning less that produces a clean run. It is deliberately not a throw: a counter whose input will not parse should fail its own row rather than take down the fifteen other counters in the same audit.' },

  { id: 'G-258', file: 'blanket-audit.mjs', anchor: "try { gen = JSON.parse(readFileSync(f, 'utf8'))._generator || JSON.parse(readFileSync(`${MAPS_DIR}/433d.map.json`, 'utf8'))._generator; } catch { gen = null; }", verdict: 'sound',
    why: 'null IS A FAILING VERDICT AND THE NEXT LINE READS IT AS ONE. `return !!gen && ...` makes an artefact that will not parse, or that names no generator, UNCOVERED -- so the unreadable case and the undeclared case produce the same row, because for [K-26]\u2019s claim they are the same fact: the ground that something re-derives this file cannot be verified. What is not possible is silence. The fallback to the map\u2019s own `_generator` is not a widening: all three 433-D construct files are written by ONE generator in one run and `--check` regenerates all three and compares, so the totals and headings files are covered by the map\u2019s declaration rather than by a declaration of their own.' },

  { id: 'G-259', file: 'assert-examined.mjs', anchor: "onlyWhen: (f) => { try { const m = JSON.parse(readFileSync(`adapters/pdf/maps/${f}.map.json`, 'utf8')); return Object.keys(m.groups || {}).filter((k) => !k.startsWith('_')).length > 0; } catch { return true; } }", verdict: 'sound',
    why: 'THE CATCH RETURNS TRUE, WHICH IS THE DEMANDING DIRECTION, AND THAT IS THE WHOLE OF ITS DISPOSITION. This predicate decides whether adapters/pdf/assert-overflow.mjs OWES an examined count on a form -- it does only where the map declares a repeatable group, because the tool itself refuses a groupless form in as many words. An unreadable map yields TRUE, so the guard is still required to speak and the pair is reported NOT REPORTED, which stops the run. `catch { return false }` is the shape that would be quiet: a map that could not be read would quietly excuse the tool from reporting on that form, and an excuse resting on a file nobody could open is an exclusion nobody wrote ([R-14]). The population it decides over is derived from MAPPED_FORMS() and never narrowed by this predicate -- only the OWED set moves, and every form it moves out of is named with its reason on every run.' },

  { id: 'G-260', file: 'assert-y-convention.mjs', anchor: "} catch { /* an unreadable declaration falls through to the spawn, which will report it */ }", verdict: 'sound',
    why: 'THE CATCH FALLS THROUGH TO THE MORE DEMANDING PATH, NOT PAST IT. The block above it reads a headings declaration and returns an EMPTY population when the file declares zero headings -- which is a real state on 433-D, whose map declares no repeatable group and whose headings file therefore declares no heading, checked in both directions by adapters/pdf/verify-headings.mjs. If the declaration will not parse, this catch does NOT return the empty population: control continues to the spawn, verify-headings.mjs is run, and the existing UNREADABLE branch reports "emitted no HEADING-Y line for <form> though <form>.headings.json exists. The reading did not happen; it did not pass." So an unreadable file and a file declaring zero headings reach different verdicts, which is exactly the distinction this file exists for, and the quiet direction is the one the catch does not take.' },

  { id: 'G-239', file: 'assert-subject-class.mjs', anchor: "catch (e) { return { stop: `${p} will not parse: ${e.message}. An unreadable declaration is not an absent one.` }; }", verdict: 'sound',
    why: 'AN UNREADABLE DECLARATION IS RETURNED AS A STOP, NOT AS AN ABSENCE, WHICH IS THE ONLY REASON THIS CATCH IS NOT THE DEFECT THE SWEEP IS LOOKING FOR. The empty input here is a subject-class file that will not parse, and the tempting shape -- `catch { return null }` -- would make every clause examine zero stems and every one of them pass, over a form whose classes nobody could read. It returns `{ stop }`, the CLI prints it and adds to the failure count, and the run exits 2. The population is separately protected: CLASSED_FORMS() derives the form list from the maps directory, so a file that cannot be parsed still APPEARS in the list and still has to be disposed. A file that vanished from disk and a file that will not parse both end the run, and neither can be mistaken for a form with nothing to check.' },

  { id: 'G-240', file: 'subject-class.mjs', anchor: 'const hits = (text, re) => [...new Set(String(text).match(re) || [])];', verdict: 'sound',
    why: 'AN EMPTY YIELD IS THE INDEPENDENT VERDICT, SO A DEAD MATCH WOULD BE QUIET -- AND THAT IS EXACTLY WHY THE TWO REGEXES IT READS ARE SELF-ASSERTING AT LOAD. `|| []` here means "this caption names no marker of this kind", which is a real and very common answer: 77 of 433-D’s 83 cells are independent. So this site cannot be judged by its own branch and is disposed on the instrument in front of it. RX-SC-01 and RX-SC-02 are registered through adapters/pdf/regex-self-assert.mjs with six required matches and eleven required rejects between them; a pattern that stopped matching FAILS TO LOAD and no caller ever reaches this line. That is not a hypothetical protection: the first draft of the marker builder closed on a bare word boundary, could not match the printed plurals "Individuals", "Wage Earners" and "Business Owners", and the module refused to import -- which is the empty-yield case being caught before it could be quiet.' },

  { id: 'G-241', file: 'subject-class.mjs', anchor: "if (individual.length && entity.length) return { class: 'dependent', side: null, individual, entity };", verdict: 'sound',
    why: 'THE `length &&` SHAPE THAT [R-17] IS WRITTEN ABOUT, AND HERE IT IS EXHAUSTIVE RATHER THAN CONDITIONAL. In `if (nums.length && mismatch)` the empty case falls into a silence that reports PASS. This expression has no silence to fall into: it is the first of four total branches over the same two lists, and BOTH BEING EMPTY IS ITSELF A NAMED VERDICT -- independent -- returned two lines further down. Every input reaches exactly one of dependent, conditional:individual, conditional:entity, independent, and the caller’s fourth state, a cell with NO class, is a STOP asserted at [SC-1] rather than a value this function can produce. The three-way split is precisely the repair for a two-way test that dropped its exclusive-or into an else branch, so collapsing it back into a guarded single case is the defect it replaced.' },
];
