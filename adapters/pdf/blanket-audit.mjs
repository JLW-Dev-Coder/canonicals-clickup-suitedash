...
//   5. THE CANARY IS NOT VISIBLE TO ITS OWN DETECTOR — [R-42], and it is the newest of the five.
//      Rule 1 says the canary must not be drawn from the artefacts. This is the other direction:
//      the artefacts must not be able to draw the canary. adapters/pdf/assert-name-sets-imported.mjs
//      planted `export const FOO_SET = [...]` as a literal in its own source, its own declaration
//      regex matched it while sweeping adapters/pdf, and it reported 52 declarations in a tree
//      holding 51. A canary is ASSEMBLED — concatenated, or built at run time — so the pattern the
//      detector looks for never appears literally in the file that carries the plant. Build the
//      next canary against [R-42]; do not rediscover it at a wrong count.
//
...
export const DETECTORS = {
  ...
  'assert-runner-derivation.mjs': { canary: "FOUR PLANTED RUNNER SOURCES run through the same classify() the sweep runs over the real runner, each with an asserted problem count, and none drawn from the artefacts. The three failure directions are the ones that go quiet: an UNREGISTERED PROBE must yield exactly one (a probe nobody claimed is how `overflow 433d` got invented in the first place), a MISSING REQUIRED ANCHOR must yield exactly one (the fix for two of the four sites REMOVED an inference, and a sweep that only looks for probes reads a removal as a clean file — [D-12]'s shape, a canary over the comparator and not the population selector), and an ORPHANED ANCHOR must yield exactly one. The FOURTH is a conforming source that must yield NOTHING, because a sweep that reported everything would satisfy a failure-only canary and stop the engine on its next run. PROVED ON ITS FIRST RUN, AND NOT AS A DRILL: the missing-required case expected 1 and got 2, because an absent `required` anchor was reported once as AUTHORITY NOT CONSULTED and again as an orphan. Two counts for one defect, in the direction that INFLATES a problem list, so nothing would have failed and nobody would have looked. The orphan pass is now over probe entries only. [R-42] IS HELD TWICE HERE. The population is one file named by package.json's regression:closing script, and that the population is not this file's own source is ASSERTED rather than left to prose; and the probe tokens inside the plants are assembled by concatenation, so the literal text the detector matches never appears in the file carrying the plants even if the population is ever widened to the directory." },
  'assert-name-sets-imported.mjs': { canary: 'CANARY, five planted source files run through the same detect() the sweep runs over the tree, with an asserted yield of exactly ONE declaration and exactly ONE hit, both compared by key. The five exercise the distinctions the predicate rests on: a.mjs DECLARES the set; b.mjs retypes it and must be the one hit; c.mjs imports it AND holds the same literal and must not be; d.mjs holds a SUPERSET and e.mjs a SUBSET, and neither must be. The last two are the direction the first draft got wrong — it tested containment rather than set equality, reported twenty-odd phantom sites and named this very file among them — so a regression there dies here rather than being noticed by a reader. The planted source is ASSEMBLED by concatenation so the literal declaration text does not appear in this file, which is [R-42] and this is the defect that earned it: a canary that entered the population it guards would make the sweep report 52 declarations in a tree holding 51. Nothing in it is drawn from the artefacts, and the run exits 5 without touching the tree if it does not come back.' },
  ...
  'xfa-fieldset.mjs': { canary: 'canary(), TWELVE planted cases run through the SAME extractFields() the derivation calls, over a synthetic XFA template written in the file and drawn from no artefact. Both directions throughout, because this walker can fail in either: an exact field count of five catches a walker that dropped a subform AND one that counted the planted <draw> as a field; every `(no bind element)` and `(no ui element)` case asserts that an absent element is RECORDED rather than defaulted, since a default would make a field that refuses data look like one that accepts it. THE CASE THAT MATTERS MOST IS `unnamed-subform-is-transparent`: XFA gives an unnamed subform no level in the SOM path, and a walker that invented a placeholder level would produce paths that read perfectly and that no processor would resolve — and nothing downstream would ever say so, because the artefact would regenerate byte-identical from the same wrong walk forever. That is [D-12]\'s shape, a canary over the comparator rather than over the thing that builds the population, and it is the direction this one is aimed at. Two further cases carry the checks in build(): the raw `<field` count must AGREE with the walk on a known-good input, which is what makes the disagreement check evidence at all, and an empty template must yield an empty set, which is the input to the zero-is-a-STOP guard at guard-sweep [G-276]. The canary runs FIRST and unconditionally in the CLI, before any artefact is written or compared. PROVED TWICE, AND THE FIRST TIME WAS NOT A DRILL. (a) On its very first run the canary FAILED, and the CLAIM was wrong rather than the walker: the field count was written as five — the number of `<subform>` blocks in the plant — and derived four, because one of those blocks holds the planted draw and a draw is not a field. The claim was corrected and the comparison was not loosened, which is [G-184]\'s shape. (b) Then broken deliberately: with the unnamed-subform branch changed to concat a `(unnamed)` placeholder level, the run reported 2 of 12 failed — the transparency case, and the `bind match="none"` case beside it, because the invented level moved the SOM key the second case looks its field up by — and exited 2. Restored, 12 of 12.' },
