...
---
## [R-38] A cycle that has not run its own closing regression has not ended

> The closing regression is part of LANDING, not part of reporting. The next cycle's pre-flight
> is a backstop, not a substitute — a backstop tells you a cycle ago something broke; it cannot
> tell the cycle that broke it.

**The defect that earned it.** `npm run sweeps:deep` sat **red for a whole cycle** and nothing
surfaced it, because the cycle that broke it did not run it. `scratchpad/p55-carried-dm2-d24.mjs`
resolved `[DM-2]` by patching `adapters/pdf/maps/433d.map.json` — the ARTEFACT — and that file is
generated by `scratchpad/p54-433d-author-map.mjs`, which is the last step of `sweeps:deep`. So the
sweep said, correctly and to nobody, *"433d.map.json does not match what re-derivation produces"*.
It was found by the NEXT cycle's pre-flight and repaired in `137a22b`, which put the resolution in
the generator and re-derived the committed bytes.

**That is a gap in how a cycle ENDS, not a gap in any tool.** Every instrument involved worked:
the generator declared itself `[R-19]`, the checker compared re-derivation to disk, and the sweep
was wired. The one thing missing was the run, and the run was missing because "the work is done"
and "the work is proved" had been allowed to be the same moment.

**The second half of the rule, and it is this cycle's own contribution.** A pre-flight that finds
a red sweep must establish WHOSE red it is before reporting it, because an instrument can be
broken by the run reading it. On this cycle `sweeps:deep` came back exit 2 with
`assert-fixture-authorship` reporting NOT REPORTED on all six forms — and the cause was three
`declaration-coverage` runs executing CONCURRENTLY with `npm run sweeps`, each of which reaches
`assert-fixture-authorship` through `validate-map.mjs`, and each of which holds the whole of
`samples/` in memory and restores it in a `finally`. Two such holds interleave and the later
restore writes the earlier process's snapshot back permanently:
`samples/433boi.slice3.sample.json` was left carrying `author-record-shape-fixtures.mjs`'s output
under `author-slice3-fixture.mjs`'s name, in the working tree, silently. It is registered as
`[D-27]`. Read on a clean tree the sweep is exit 0 in ~700s, which is what `137a22b` recorded.
**A red reported without establishing whose red it is, is a state claim, which is `[R-01]`.**

**Roughly when.** Ruled 2026-08-26, prompt 57 ruling 2, on the `sweeps:deep` account in
`137a22b`. Cycle-dated: the commit that lands it is the commit this rule is written in, so it has
no hash yet.

---

## [R-39] Sources written by one authoring tool into one file are not independent witnesses

> Corroboration requires independent ORIGIN, not repetition. N statements produced by one build
> step are one statement quoted N times, and counting them as N is the arithmetic that makes a
> guess look settled.

**The defect that earned it.** 433-H's single drawn page is the Adobe Reader placeholder: it
prints no revision, no catalogue number and no form number, so the standing check — revision and
catalogue number read from the drawn page — has nothing to read. **Four sources agree on
`Rev. 3-2025` and `71232U`**: the XFA template's `product_id` draw, its `assist`/`speak`
accessibility text, the PDF `/Info` title, and the build path inside the config packet. Four
agreeing sources is the shape of a settled fact, and here it is one fact — **all four were written
by one authoring tool into one file, in one build.** If that build stamped the wrong revision, all
four are wrong together and agree perfectly. Nothing about their agreement could have contradicted
the conclusion, which is `[R-05]`'s shape in a different medium.

**What follows is not "distrust the four" but "say what they are".** The honest move is `[R-37]`'s
second half: declare that the revision cannot be pinned from drawn bytes, and pin the DOCUMENT
instead, by a SHA the tree actually opens. A pin that answers a weaker question loudly beats a pin
that answers the right question silently.

**The rule is general and this engine has the other instances.** `[D-06]`'s five columns were
"three load-bearing, two inert" on five readings of one map. `[R-19]`'s three field-file
generators share one output shape, so a file written by the wrong one looks exactly like a file
written by the right one. Agreement between things with a common author is not evidence about the
world; it is evidence about the author.

**Roughly when.** Ruled 2026-08-26, prompt 56 ruling 4, adopted as a general rule about
corroboration in prompt 57. Cycle-dated: the commit that lands it is the commit this rule is
written in, so it has no hash yet.
