...
export const MANUAL = [
  ...
  { id: 'MR-22', tool: 'adapters/hubspot/hs-describe-reused-433d.mjs', ground: 'writes',
    reason: 'WRITES shared property descriptions on the live portal — [MR-7]\x27s ground, on the first form that reuses from TWO creators. It APPENDS to a live description rather than replacing it, because two of its three targets carry the backbone convention ("Shared across the 433 series - named for the fact, not the form") and overwriting that to satisfy a later convention is what [R-21] refuses; the append is asserted still present in what the portal hands back, on a separate read. Its effect is guarded by rerun-regression.mjs, which re-runs every deriver against the descriptions it leaves behind — that is [D-18]\x27s fourth instance, where one form\x27s describe pass broke another form\x27s deriver seven prompts later and nothing said so. The writer itself stays manual, and it is deliberately NOT wired to an npm script even in its --dry-run form: a tool in a suite is a tool somebody runs without deciding to.' },
