...
export const REGISTER = {
  ...
  'adapters/pdf/assert-runner-derivation.mjs': { kind: 'engine-wide', why: "Its universe is ONE FILE — the closing-regression runner, resolved from package.json’s regression:closing script rather than from a path it carries ([R-22]) — and the question is whether every filesystem probe in it is registered with the authority the fact is derived from. That is a fact about one runner and about package.json, not about any form. IT IS IN THIS POPULATION ONLY BECAUSE IT NAMES MAPPED_FORMS() IN PROSE: two DERIVATIONS entries record that the runner’s form universe now comes from MAPPED_FORMS() instead of a naming pattern over adapters/pdf/forms, and the per-form signature’s first clause matches that mention. It never calls it. That is the same false positive assert-reachability.mjs guards against in the other direction — a tool named in a MESSAGE or a REGISTER must yield no edge — and it is disposed here rather than dodged by rewording the register, because a register entry written to avoid a classifier is a register entry that says something other than what is true. It owes no per-form examined count and reports two engine-scoped figures on every run: the number of filesystem probe sites found in the runner and the number of register entries disposing of them, with a probe the register does not claim taken down as a STOP rather than reported as a clean sweep." },
