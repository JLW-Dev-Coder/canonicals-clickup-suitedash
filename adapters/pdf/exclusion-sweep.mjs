...
export const PREDICATES = [
  ...
  { id: 'EX-10', pred: 'isProse', definedIn: ['adapters/pdf/exclusion-sweep.mjs', 'adapters/pdf/guard-sweep.mjs', 'adapters/pdf/success-sweep.mjs', 'adapters/hubspot/gen-fields-from-map.mjs', 'adapters/hubspot/post-pass-sweep.mjs'], kind: 'structural',
  ...
  { id: 'EX-39', pred: 'SB23_FILES', definedIn: 'adapters/pdf/sweep-boundary.mjs', kind: 'structural',
    what: 'Selects the map artefacts [SB-23] stands over — every unswept adapters/pdf/maps/*.json that DECLARES a generator — and is subtracted by [SB-25] so the two entries cannot both claim one file.',
    structural_because: "IT PARTITIONS THE MAPS DIRECTORY AND EVERY PART IS CLAIMED BY SOMETHING, WHICH [SB-92] DERIVES ON EVERY RUN. A file this predicate SELECTS is [SB-23]’s, and that entry then demands its generator exists and that a standing npm script runs it. A file it REJECTS falls to [SB-24] if it is an intake record, to [SB-25] if it is a hand-maintained cross-form register, to count-sweep’s own manifest if it is a swept per-form artefact — and to NOTHING otherwise, which is the state [SB-92] refuses BY NAME as an UNCLAIMED FILE. So this predicate excuses nothing: it routes, and the residual is what proves every route has a destination. Its own failure directions both land in the loud place. A predicate that selected NOTHING would leave 433d.mirror.json and 433d.subject-classes.json unclaimed and stop the run; one that selected EVERYTHING would take the intake records and the cross-form registers into [SB-23], whose crosscheck would then demand a standing generator for files that have none — 433d.pairs.json names a one-shot in scratchpad/ that no npm script runs — and stop the run there instead. It is also the remedy for its OWN predecessor: [SB-23] used to select by two filename spellings, which is the object [SB-24]’s `433d.` was, and reading the declaration instead is what [D-24] establishes as the difference between a population and a list of what somebody looked at." },

  { id: 'EX-40', pred: 'isND', definedIn: 'adapters/hubspot/cf-mirror-export-render.mjs', kind: 'structural',
    what: "Separates a derived field carrying the not-derivable marker from one carrying a value, in the markdown and CSV renderings of the mirror export and in the checker that re-derives them.",
    structural_because: "IT ROUTES BETWEEN TWO RENDERINGS AND EXCUSES NOTHING FROM ANY CHECK, AND THE WHOLE POINT OF THE EXPORT IS THAT NEITHER BRANCH IS BLANK. A derived field is either {value, source} or {not_derivable:true, _reason}: the TRUE branch prints the reason nobody could derive it, the FALSE branch prints the value and the file it came from. There is no third rendering and no branch that omits the field, which is [R-07] applied to a field rather than to a count — a value that cannot be worked out is written as the reason it cannot, never left out and never guessed from the prefix tag. Nothing leaves an assertion by it. adapters/hubspot/check-cf-mirror-export.mjs check [8] walks every derived field of every record through this same predicate, imported rather than re-implemented, and REQUIRES that the true branch carry a non-empty _reason string and the false branch carry both a value key and a non-empty source — so a field either predicate branch would render as empty is a STOP naming the record and the field. Both failure directions are loud. A version answering TRUE to everything would make check [8] demand a _reason on 959 records that carry values and fail on the first; one answering FALSE to everything would demand a source on the 1,960 not-derivable fields and fail on the first of those. It is also the reason the two renderings cannot drift from the JSON: builder and checker render through this one module ([R-39]), so there is one implementation of the branch and not two that agree today." },
  ...
  // ─── which tools the standing suite runs [R-34] ───────────────────────────────────────
  { id: 'EX-38', pred: 'exercised', definedIn: 'adapters/pdf/assert-reachability.mjs', kind: 'structural',
    what: 'Separates a tool the standing suite RUNS from one it does not, so that the manual register is asked only of the ones nothing runs.',
    structural_because: 'IT IS THE ASSERTION ITSELF AND BOTH BRANCHES ARE REPORTED. It is called twice over the same derived population: once to count what is run, and once to build the unreached list every member of which must then be REGISTERED with a ground the tree bears out. Nothing leaves the assertion by it — a tool it answers false for moves TOWARDS the problem state, not away from it, and the two branches partition the population, whose size is derived and printed on every run beside the unreached count. It makes no claim about the world: it reads the file\'s own shape (does its body sit behind a CLI guard) and the graph built from package.json and from spawn and import call sites, and nothing else. Both failure directions are loud and both are canaried. A version answering TRUE to everything empties the unreached list, and the register check catches it from the other side in the same run — every one of the 20 manual entries then names a tool reported as RUN, which is a STOP per entry. A version answering FALSE to everything reports all 85 tools unrun and unregistered. And the graph underneath it is planted with six fixed classifications in both directions, because the two drafts that got it wrong each certified correlate-labels.mjs — the tool [D-22] records nothing running — as reachable.' },

  // ─── the halt that survives a portal request [D-20] ───────────────────────────────────
  { id: 'EX-37', pred: 'isStop', definedIn: 'adapters/hubspot/hs-lib.mjs', kind: 'structural',
    what: 'Separates the StopSignal that stop() throws to halt a tool with a declared exit code from every other error — inside hs-lib.mjs\'s own error handler, and inside every catch site in adapters/hubspot/ that stands between a stop() and the top level.',
    structural_because: 'IT SEPARATES THIS ENGINE\'S OWN HALT FROM A FAILURE, AND IT MAKES NO CLAIM ABOUT THE WORLD. The token it tests is a MODULE-LOCAL Symbol that only stop() ever sets, so the predicate cannot be satisfied by an error from a library, from the runtime, or from any tool here — it is true of exactly the objects this module manufactured, which is a fact about identity rather than a judgement about a condition. Nothing is excused from any assertion by it. At every catch site the TRUE branch RE-THROWS, which is the loud direction and is precisely the uninterceptable halt `process.exit()` had before [D-20] forced the change; the FALSE branch runs the catch body that was already there, unchanged. In hs-lib\'s own handler the true branch swallows a signal whose exit code is ALREADY SET on process.exitCode before the throw, and the false branch prints the stack and sets code 1 — so neither branch can mask a failure as a pass. Both failure directions are canaried in adapters/hubspot/assert-exit-codes.mjs on every run: a version answering TRUE to everything would swallow a real error, and the canary plants one and asserts it is printed and the code non-zero; a version answering FALSE to everything would let a StopSignal escape as an uncaught error, and the canary plants that and asserts the declared code arrives intact.' },

  // ─── the declared record shape ────────────────────────────────────────────────────────
  { id: 'EX-36', pred: 'roleOf', definedIn: 'adapters/hubspot/assert-registry-targets.mjs', kind: 'structural',
    what: 'Separates a definition register GENERATED from a closed map from a HAND-AUTHORED one, so that [RT-1] — "this row\'s form has no generated rival" — is asked of hand-authored registers only.',
    structural_because: 'It reads the register\'s own `meta.generated_from`, which [R-19] requires every generated artefact to declare, and makes no claim about the world. Its failure direction is loud rather than quiet: a generated file misread as hand-authored reports itself as its own rival and the run STOPs — which is exactly what the first draft of this file did, printing seven problems on a tree with two. A hand-authored file misread as generated would be the quiet direction, and it cannot arise from a missing field: `meta.generated_from` has to be PRESENT for that reading, and nothing writes it but a generator.' },
