...
export function computeModulePath(params: {
  ...
  const { body_type, platform, sd_item_id } = params;
  const itemRoot = resolve(CANONICALS_DIR, platform, String(sd_item_id));

  if (body_type === 'stack-css' || body_type === 'stack-js' || body_type === 'stack-html') {
    if (!params.parent_stack_slug) {
      throw new Error(`parent_stack_slug required for body_type=${body_type}`);
    }
    const suffix = body_type === 'stack-css' ? 'CSS' : body_type === 'stack-js' ? 'JS' : 'HTML';
  ...
  if (body_type.match(/^m\d{3}-/)) {
    return resolve(itemRoot, 'order', `${body_type}.md`);
  }
  ...
  throw new Error(`Unknown body_type: ${body_type}`);
}
