...
E.push({
  ...
  rejected_candidates: [{ name: 'irs433_employer_identification_number', why: 'DECLINED, AND THE RULING IS NOW A STRUCTURE THE DERIVER READS RATHER THAN A SENTENCE INSIDE scope_reason. irs433_employer_identification_number is live, contributed by 433-A line 54 and 433-A(OIC) s4_business_ein, and on both of those forms the taxpayer is a NATURAL PERSON whose EIN belongs to a business that person owns — the business is not the taxpayer. 433-A page 7 prints the boundary itself: "Sections 6 and 7 must be completed only if you are SELF-EMPLOYED." drawn beside "No, Complete Form 433-B." irs433boi_employer_identification_number is the EIN of the entity that IS the taxpayer, and this key exists ONLY on the entity branch of 433-D’s subject route, so its subject is fixed as that entity. The state of the world in which the two differ is ordinary: a sole proprietor with an EIN files a 433-A and their company later files a 433-D, and one property would have to hold both at one moment. The prose of this ruling already stood in scope_reason and was read by a person; what it lacked was a form the twin check could read, which is [D-28].' }],
...
const doc = {
  ...
  entries: E.map((e) => ({
    ...
    // [D-28] — WHAT A REUSE DECLINED, not only what it took. A reuse names the property it
    // binds and says nothing about the live near-identical names it passed over, and the cost
    // of the wrong pass is a wrong value read back forever under a name that denies it. This
    // is projected as a STRUCTURE because adapters/hubspot/twin-check.mjs reads it by name;
    // the same ruling stated only inside scope_reason is prose, and a guard that recognised a
    // ruling by matching English could be satisfied by a sentence that happened to contain one.
    ...(e.rejected_candidates ? { rejected_candidates: e.rejected_candidates } : {}),
    why: e.why,
